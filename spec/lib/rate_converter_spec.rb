  ...
  describe '#convert' do
    context 'when converting from USD' do
      it 'converts 10 USD to EUR' do
        expect(converter.convert(10, base: 'USD', target: 'EUR')).to eq({ rate: 2, to_value: 20.0 })
      end

      it 'converts 10 USD to BRL' do
        expect(converter.convert(10, base: 'USD', target: 'BRL')).to eq({ rate: 4.0, to_value: 40.0 })
      end

      it 'converts 10 USD to JPY' do
        expect(converter.convert(10, base: 'USD', target: 'JPY')).to eq({ rate: 8.0, to_value: 80.0 })
      end
    end

    context 'when converting from BRL' do
      it 'converts 10 BRL to USD' do
        expect(converter.convert(10, base: 'BRL', target: 'USD')).to eq({ rate: 1, to_value: 2.5 })
      end

      it 'converts 10 BRL to EUR' do
        expect(converter.convert(10, base: 'BRL', target: 'EUR')).to eq({ rate: 2, to_value: 5.0 })
      end

      it 'converts 10 BRL to JPY' do
        expect(converter.convert(10, base: 'BRL', target: 'JPY')).to eq({ rate: 8.0, to_value: 20.0 })
      end
    end

    context 'when converting from EUR' do
      it 'converts 10 EUR to USD' do
        expect(converter.convert(10, base: 'EUR', target: 'USD')).to eq({ rate: 1, to_value: 5.0 })
      end

      it 'converts 10 EUR to BRL' do
        expect(converter.convert(10, base: 'EUR', target: 'BRL')).to eq({ rate: 4, to_value: 20.0 })
      end

      it 'converts 10 EUR to JPY' do
        expect(converter.convert(10, base: 'EUR', target: 'JPY')).to eq({ rate: 8, to_value: 40.0 })
      end
    end

    context 'when converting from JPY' do
      it 'converts 10 JPY to USD' do
        expect(converter.convert(10, base: 'JPY', target: 'USD')).to eq({ rate: 1, to_value: 1.25 })
      end

      it 'converts 10 JPY to BRL' do
        expect(converter.convert(10, base: 'JPY', target: 'BRL')).to eq({ rate: 4, to_value: 5 })
      end

      it 'converts 10 JPY to EUR' do
        expect(converter.convert(10, base: 'JPY', target: 'EUR')).to eq({ rate: 2, to_value: 2.5 })
      end
    end
